...
1. **Five cards, not three.** The brief described three images; node `914:18698`
   ...
   follows the node.
2. **The photograph is node `926:22119`** (`image 20493`), supplied directly by
   the repository owner as a 2x frame export — `www.figma.com` is not on the
   build runner's egress allowlist, so it could not be fetched here. Because it
   is a frame export, the crop and the 64px corners are already in the file;
   see `build/explore-programs/assets/README.md` before changing it.
